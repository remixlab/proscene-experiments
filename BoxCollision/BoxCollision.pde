...
void setup() {
  ...
  models = new InteractiveFrame[50];
  ...
  for(int i = 0; i < models.length; i++){
    models[i] = new InteractiveFrame(scene, drawRandomPolygon(10));
...
public void drawCube(int number){
  InteractiveFrame m = models[number];
...
public void checkCubeCollisions(int number, int candidate){
  InteractiveFrame player = models[number];
...
public float[] boundSphere(InteractiveFrame m) {
